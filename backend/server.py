...
async def update_product(product_id: str, payload: ProductUpdate, _: dict = Depends(require_admin)):
    ...
    # If clearing images, also clear image_url
    if "images" in update and (not update["images"]):
        update["image_url"] = ""
...
async def upload_product_image(product_id: str, file: UploadFile = File(...), _: dict = Depends(require_admin)):
    ...
    # FIX: Use ONLY the images array, don't fall back to image_url
    ...
    # Always clear image_url to avoid conflicts
    update = {"images": images, "image_url": ""}
...
async def delete_product_image(product_id: str, idx: int, _: dict = Depends(require_admin)):
    ...
    # Always clear image_url when deleting images
    update = {
        "images": images,
        "image_url": ""  # Clear completely
    }
    await db.products.update_one({"id": product_id}, {"$set": update})
